...
DD_min_lam <- round(min(subset(DDcorrs, Parameter == "Lambda")$`50%`), digits = 3)
DD_max_lam <- round(max(subset(DDcorrs, Parameter == "Lambda")$`50%`), digits = 3)

VRcorrs <- readRDS("Results/VR_corrCoef.rds")
samCorr_mean <- round(subset(VRcorrs, area == "Overall")[,2:3], digits = 3)
```
...
medianCorr <- round(cor(GT_R0$Median, GT_elas$Median), digits = 3)

minIdx <- which(GT$Median == min(GT$Median))
maxIdx <- which(GT$Median == max(GT$Median))

GT_min <- round(GT[minIdx, c("Median", "lCI", "uCI")], digits = 3)
GT_max <- round(GT[maxIdx, c("Median", "lCI", "uCI")], digits = 3)
